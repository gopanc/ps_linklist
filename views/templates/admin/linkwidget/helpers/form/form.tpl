...
{block name="input_row"}
    ...
    {elseif $input.type == 'product_pages' || $input.type == 'static_pages' || $input.type == 'account_pages'}
                  ...
                  {foreach $cms_category.pages as $key => $page}
                    <tr {if $key%2}class="alt_row"{/if}>
                      <td>
                        ...
                        {elseif $input.type == 'account_pages'}
                          <input type="checkbox" class="cmsBox" name="{$input.name}" id="{$page.id_cms}" value="{$page.id_cms}" {if in_array($page.id_cms, $fields_value['content']['account'])}checked="checked"{/if} />
                        {/if}
                      </td>
                      <td>
                        <label class="control-label" for="{$page.id_cms}">
