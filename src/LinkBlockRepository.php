...
class LinkBlockRepository
{
    ...
    public function getAccountPages($id_lang = null)
    {
        $accounts = array();
        $accountPages = array(
            'my-account',
        ...
        foreach ($accountPages as $accountPage) {
            $meta = Meta::getMetaByPage($accountPage, ($id_lang) ? (int)$id_lang : (int)Context::getContext()->language->id);
            $accounts[] = [
                'id_cms' => $accountPage,
                'title' => $meta['title'],
            ];
    ...
    public function installFixtures()
    {
        ...
        $queries = [
            'INSERT INTO `'.$this->db_prefix.'link_block` (`id_link_block`, `id_hook`, `position`, `content`) VALUES
                (1, '.$id_hook.', 1, \'{"cms":[false],"product":["prices-drop","new-products","best-sales"],"static":[false],"account":["order-slip"]}\');'
        ];
