...
class LinkBlockPresenter {
    ...
    {
        $cmsLinks = $productLinks = $staticsLinks = $accountLinks = $customLinks = array();
        ...
        if (isset($content['account'])) {
            $accountLinks = $this->makeAccountLinks($content['account']);
        }
    ...
    private function makeAccountLinks($accountIds) {
        $accountLinks = array();
        foreach ($accountIds as $accountId) {
            if (false !== $accountId) {
                $meta = Meta::getMetaByPage($accountId, (int)$this->language->id);
                $accountLinks[] = array(
                    'id' => 'link-account-page-'.$accountId,
                    'class' => 'cms-page-link',
                    'title' => $meta['title'],
                    'description' => $meta['description'],
                    'url' => $this->link->getPageLink($accountId, true),
                );
            }
        }
